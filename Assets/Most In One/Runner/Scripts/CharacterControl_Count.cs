...
namespace Solo.MOST_IN_ONE
{
    ...
    public class CharacterControl_Count : MonoBehaviour
    {
        ...
        [Tooltip("The child holder object inside the character object")]
        [Required] public CapsuleCollider CollisionRef;
        ...
        List<CapsuleCollider> _colliderList = new();
        ...
        public void UpdateCollider()
        {
            ...
            for (int i = 0; i < ChildHolder.childCount; i++)
            {
                if (_colliderList.Count -1 < i) _colliderList.Add(CollisionHolder.AddComponent<CapsuleCollider>());
                _colliderList[i].isTrigger = CollisionRef.isTrigger;
                _colliderList[i].radius = CollisionRef.radius;
                _colliderList[i].height = CollisionRef.height;
        ...
        void UpdateConsitrains() // fix the main character collision scale, position and update road Consitrains
        {
            ...
            //GetComponent<SphereCollider>().radius = (Mathf.Abs(DistUpdate.x - DistUpdate.y) * ChildHolder.transform.localScale.x / 2) + ExtraCollisionScale;
