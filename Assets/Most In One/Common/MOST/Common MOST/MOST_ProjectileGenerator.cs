...
namespace Solo.MOST_IN_ONE
{
    ...
    public class MOST_ProjectileGenerator : MonoBehaviour
    {
        ...
        // ✅ FIX: Default LifeTime = 3, and we enforce a fallback below as well.
        [Tooltip("Directed mode only: auto-destroy spawned projectile after this time (seconds).\n" +
                 "If set to 0, a safe fallback of 3 seconds is used to prevent endless bullets.")]
        [HideIfAny(nameof(Type), DataToSend.Throw, true)]
        [Min(0)] public float LifeTime = 3f;
        ...
        [HideIfAny(nameof(AddDamageSetter), true, false)]
        [Min(0)] public int DestroyAfterGetHealth2;

        [HideIfAny(nameof(AddDamageSetter), true, false)]
        public UnityEvent OnGetHealth2;

        [HideIfAny(nameof(AddDamageSetter), true, false)]
        public UnityEvent OnGetHealth3;

        [HideIfAny(nameof(AddDamageSetter), true, false)]
        public UnityEvent OnGetHealth4;

        [HideIfAny(nameof(AddDamageSetter), true, false)]
        public UnityEvent OnGetHealth5;
        ...
        /// computes and applies offsets, then spawns and schedules destruction (Directed mode).
        /// </summary>
    ...
    // ✅ FIX: Always enforce a safe lifetime (prevents endless bullets)
    float lifeTimeToUse = LifeTime > 0f ? LifeTime : 3f;
    Destroy(spawndObj, lifeTimeToUse);
