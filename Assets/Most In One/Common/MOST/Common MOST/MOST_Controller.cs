...
namespace Solo.MOST_IN_ONE
{
    ...
    public class MOST_Controller : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
    {
        ...
        [Tooltip("Touch distance threshold (in pixels) required to consider the gesture a swipe.")]
        ...
        void Update()
        {
            ...
            if (Enable) // Main enable
            {
                ...
                if (Controls == InputControl.ControlledByScreenTouch_Pointer || Controls == InputControl.ControlledByBoth)
                    if (EnableTouch)
                    {
                        if (Type == ControllerType.Button || Type == ControllerType.MouseButton) TouchButtonDetector();
        ...
        void OnTRelease([Optional] TouchCompat touch, [Optional] PointerEventData eventData)
        {
            ...
            // (swipe detection measured in PIXELS)
            if (_isSwiped && Vector2.Distance(_swipeStartScreen, screenPos) > SwipeSinstivity)
            {
        ...
        {
            if (!Enable || !(Controls == InputControl.ControlledByScreenTouch_Pointer || Controls == InputControl.ControlledByBoth)) return;// Main enable
        ...
        {
            if (!Enable || !_isTouched || !(Controls == InputControl.ControlledByScreenTouch_Pointer || Controls == InputControl.ControlledByBoth)) return;// Main enable
            if ((Type == ControllerType.StaticJoystick || Type == ControllerType.DynamicJoystick)
