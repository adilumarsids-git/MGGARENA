    ...
    public class MOST_Spawn : MonoBehaviour
    {
        ...
        void DrawShadedAreaBetweenSquares(Vector3 center, float innerSize_x, float innerSize_z, float outerSize_x, float outerSize_z, float height, int segments, Color color)
        {
            ...
            for (int i = 1; i < segments; i++)
            {
                ...
                {
                    Vector3 leftEdge = new(center.x - outerSize_x / 2, 0, z);
                    Vector3 rightEdge = new(center.x + outerSize_x / 2, 0, z);

                    // If we're inside the inner square's z-range
                    if (z >= center.z - innerSize_z / 2 && z <= center.z + innerSize_z / 2)
                    {
                        Vector3 innerLeft = new(center.x - innerSize_x / 2, 0, z);
                        Vector3 innerRight = new(center.x + innerSize_x / 2, 0, z);
            ...
            for (int i = 1; i < segments; i++)
            {
                ...
                {
                    Vector3 bottomEdge = new(x, 0, center.z - outerSize_z / 2);
                    Vector3 topEdge = new(x, 0, center.z + outerSize_z / 2);

                    // If we're inside the inner square's x-range
                    if (x >= center.x - innerSize_x / 2 && x <= center.x + innerSize_x / 2)
                    {
                        Vector3 innerBottom = new(x, 0, center.z - innerSize_z / 2);
                        Vector3 innerTop = new(x, 0, center.z + innerSize_z / 2);
